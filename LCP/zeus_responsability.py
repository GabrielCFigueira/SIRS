...
onoff_queries = {'motor': lambda new_state: 'ok',
                 'lock': lambda new_state: 'ok',
                 'brake': lambda new_state: 'ok'}
...
def deal_with_it(query):
    ...
    if query[0] == 'read' and query[1] in read_query:
        return str(read_query[query[1]]())

    elif tuple(query) in product(onoff_queries.keys(), ['on', 'off']):
        return str(read_query[query[0]](query[1]))
